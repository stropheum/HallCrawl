...
void UFireRifleAbility::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
    ...
    if (const TObjectPtr<UHallCrawlWeaponComponent> Weapon = Character->EquippedWeapon;
        Weapon && Weapon->IsBeamWeapon())
    {
        LaserCannon = Weapon->GetLaserCannon();
        ActivateBeamWeapon();
    }
    else
    {
...
void UFireRifleAbility::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
    if (LaserCannon)
    {
...
void UFireRifleAbility::FireProjectile(const FGameplayAbilitySpecHandle Handle)
{
    ...
    if (ProjectileClass == nullptr)
    {
        ...
        UE_LOG(LogGameplayTags, Warning, TEXT("ProjectileClass == nullptr. Falling back to base class"));
    }
...
void UFireRifleAbility::ActivateBeamWeapon()
{
    if (LaserCannon)
    {
        LaserCannon->ActivateBeam(true);
    }
}

void UFireRifleAbility::DeactivateBeamWeapon()
{
    if (LaserCannon)
    {
        LaserCannon->ActivateBeam(false);
    }
}